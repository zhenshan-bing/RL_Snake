...
while ((time.time() - start_time) <= 10):
    ...
    target_position = 30 * np.sin((time.time() - start_time) * 4 * np.pi) + 0
    target_position = target_position / 180 * np.pi 
    target_position_list.append(target_position * 180 / np.pi)
    
    # this value changes the shape
    pos_before_action = sim.data.qpos[joints_idx] * 0.9
    ...
    while (j < 1):
        ...
        sim.data.ctrl[:] = error_position * 1.0 + error_position_integrated * 0.1 * 0 + vel_before_action * 0.05 * 1
...
ax.plot(step_index*10/len(sim_position_list), sim_position_list)
plt.xlim(3, 4)
plt.grid()
